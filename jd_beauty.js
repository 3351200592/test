...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            getUA();
            await accountCheck();
            if (!$.hasDone) await $.wait(3000);
            if ($.accountCheck) await jdBeauty();
        ...
        if (e.data !== 'pong' && e.data && safeGet(e.data)) {
            ...
            switch (vo.action) {
                ...
                case "shop_products":
                    ...
                    let count2 = $.taskState.product_adds.length;
                    if (count2 < 5 && ADD_CART) console.log(`去做浏览并加购任务`)
                    for (let i = 0; i < vo.data.products.length && count2 < 5 && ADD_CART; ++i) {
                        ...
                        if (!$.taskState.product_adds.includes(product.id)) {
                            ...
                            client.send(`{"msg":{"action":"write","type":"action","args":{"action_type":9,"channel":2,"source_app":2,"vender":"${product.shop_id}"}}}`);
                            client.send(`{"msg":{"action":"write","type":"action","args":{"action_type":5,"channel":2,"source_app":2,"vender":"${product.shop_id}"}}}`);
                        }
                        await $.wait(3000);
                    }
                ...
                case "produce_position_info_v2":
                    if (vo.data && vo.data.material_name !== '') {
                        ...
                        if (vo.data && vo.data.valid_electric > 0) {
                            console.log(`【${vo.data.position}】尚未开始生产`)
                            let ma
...
function getIsvToken2() {
    let config = {
        url: 'https://api.m.jd.com//client.action?functionId=isvObfuscator&clientVersion=10.2.2&build=91077&client=android&partner=au_jdjlhdqj073&eid=eidA91634120c3l5447e82e8d92839a10582393a7a16ac663f5f33b3FKGkwQFy5On8XXRWQ+9gLT2lwAojLkcFcA2WNFfiK1Vkq450zigksvEbfPm5&sdkVersion=28&lang=zh_CN&harmonyOs=0&networkType=UNKNOWN&uts=0f31TVRjBStT8DJ4TwxFctRlEX8jM8POtnfSQNlVhaYFweFjmr2l7GO4uRuqZt%2F9k2E3Xdh9sdIWuitFvBEk9i8xUWxpuxO9AwOGeYo5tEgTDZpClVC8xRubP9%2FtSUg%2B7H9mMVeOscWE7VyyJ0UW4zq1%2BALNIqRUyYbdE%2FPi5uoZpdFothHMjBgbk2vPZCoSP8426KRN3468dnrGvnCnSQ%3D%3D&uemps=0-0&ext=%7B%22prstate%22%3A%220%22%7D&ef=1&ep=%7B%22hdid%22%3A%22JM9F1ywUPwflvMIpYPok0tt5k9kW4ArJEU3lfLhxBqw%3D%22%2C%22ts%22%3A1636946303173%2C%22ridx%22%3A-1%2C%22cipher%22%3A%7B%22d_model%22%3A%22Jw90ZJu%3D%22%2C%22wifiBssid%22%3A%22dW5hbw93bq%3D%3D%22%2C%22osVersion%22%3A%22EG%3D%3D%22%2C%22d_brand%22%3A%22bWVfoxU%3D%22%2C%22screen%22%3A%22CtO5DMenCNqm%22%2C%22uuid%22%3A%22ZJO5CJPsDJK5CtPwYWUzYG%3D%3D%22%2C%22aid%22%3A%22ZJO5CJPsDJK5CtPwYWUzYG%3D%3D%22%7D%2C%22ciphertype%22%3A5%2C%22version%22%3A%221.2.0%22%2C%22appname%22%3A%22com.jingdong.app.mall%22%7D&st=1636946407445&sign=58ec9cc9852e25c72f39d21b3682ff8a&sv=111',
