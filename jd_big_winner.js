...
function gambleOpenReward() {
    ...
    const options = {
        ...
        headers,
        body: `functionId=gambleOpenReward&body=${JSON.stringify(body)}&t=${Date.now()}&appid=activities_platform&clientVersion=3.8.16`
    }
    return new Promise(resolve => {
                ...
                if (err) {
                    ...
                    if (data) {
                        ...
                        if (data['code'] === 0) {
                            ...
                            if (data.code === 20007 || data.code === 1000 || data.code === 20009) {
                                $.canOpenRed = false;
...
function openRedReward(functionId = 'gambleChangeReward', type) {
    ...
    const options = {
        url: `https://api.m.jd.com/`,
        headers,
        body: `functionId=${functionId}&body=${JSON.stringify(body)}&t=${Date.now()}&appid=activities_platform&clientVersion=3.8.16`
    }
    return new Promise(resolve => {
        $.post(options, (err, resp, data) => {
            try {
...
function apCashWithDraw(id, poolBaseId, prizeGroupId, prizeBaseId, prizeType) {
    ...
    const options = {
        url: `https://api.m.jd.com/`,
        headers,
        body: `functionId=apCashWithDraw&body=${encodeURIComponent(JSON.stringify(body))}&t=${Date.now()}&appid=activities_platform&clientVersion=3.8.16`
    }
    return new Promise(resolve => {
        $.post(options, (err, resp, data) => {
            try {
