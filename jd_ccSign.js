...
async function ccSign(functionId, body) {
    let sign = await getSign(functionId, body)
    if (!sign) { console.log(`ccSign:获取sign失败`); return }
    return new Promise(async resolve => {
        $.post(taskUrl(functionId, sign), async (err, resp, data) => {
            try {
                if (err) {
...
async function getsecretPin(pin) {
    var key = CryptoJS.enc.Utf8.parse(CryptoJS.MD5(pin).toString().substr(8, 16));
    var srcs = CryptoJS.enc.Utf8.parse(pin);
