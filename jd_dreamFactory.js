...
let outpath = './DreamFactory_HelpOut.json'
...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            if ((cookiesArr && cookiesArr.length >= ($.tuanNum || 5)) && $.canHelp) {
                console.log(`\n=========== 开始【京东账号${$.index}】${$.nickName || $.UserName} 内部相互进团 ===========`);
                if ($.TuanhelpRunout.includes($.index) || $.TuanhelpOut.includes($.UserName)) {
                    console.log("\n助力次数耗尽、不执行此账号. . .")
                    continue
                }
    ...
    thefs.writeFile(Tuanoutpath, JSON.stringify($.TuanHelpOuts), function (err) {
        if (err) console.log(`\n\n写入参团缓存失败：${err}\n`)
        else console.log("\n\n写入参团缓存成功 . . .")
    })
    await $.wait(3000)
...
async function helpFriends() {
    ...
    for (let v of newShareCodes) {
        ...
        let assistFriendRes = await assistFriend(code);
        if (assistFriendRes) {
            if (assistFriendRes.msg && assistFriendRes.msg.indexOf("好友的工厂的工人已经满员") > -1) {
                ...
                if ($.HelpOuts.helpFull.indexOf($.theName) == -1) $.HelpOuts.helpFull.push($.theName) // 已满的写入缓存
            } else if (assistFriendRes.msg && assistFriendRes.msg.indexOf("好友的工厂暂未开启") > -1) {
                if (checkArr($.myCodes, code) > -1) $.myCodes.splice(checkArr($.myCodes, code), 1) // 剔除助力已满的助力码
                if (checkArr($.otherCodes, code) > -1) $.otherCodes.splice(checkArr($.otherCodes, code), 1) // 剔除助力已满的助力码，未开启的不写入缓存，万一又开启了呢
            }

            if (assistFriendRes && assistFriendRes['ret'] === 0) {
                console.log(`助力好友【${$.theName}】成功，一次只能助力一个，助力耗尽，故跳出助力\n`)
                if (!$.helpRunout.includes($.index)) $.helpRunout.push($.index) //助力耗尽实时过滤
                if ($.HelpOuts.helpOut.indexOf($.UserName) == -1) $.HelpOuts.helpOut.push($.UserName) //助力耗尽缓存当天过滤
                break
            } else if (assistFriendRes && assistFriendRes['ret'] === 11009) {
                console.log(`助力好友【${$.theName}】失败：${assistFriendRes.msg}，跳出助力\n`);
                break
            } else {
                console.log(`助力好友【${$.theName}】失败：${assistFriendRes.msg}\n`)
            }
        }
    }
}
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data['ret'] == 0 && data.data) {
                            ...
                            if (data && data.msg && data.msg.indexOf('登录京东账号才可以') > -1) {
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data['ret'] === 0) {
                            ...
                            if (!$.TuanhelpRunout.includes($.index)) $.TuanhelpRunout.push($.index)
                            if ($.TuanHelpOuts.helpOut.indexOf($.UserName) == -1) $.TuanHelpOuts.helpOut.push($.UserName)
                        } else if (data['ret'] === 10005 || data['ret'] === 10206) {
                            //火爆，或者今日参团机会已耗尽
                            console.log(`参团失败：${JSON.stringify(data)}\n`);
                            $.canHelp = false;
                            if (data && data.msg && /(助力次数已用完|您已成功参团|活动火爆)/g.test(data.msg)) {
                                if (!$.TuanhelpRunout.includes($.index)) $.TuanhelpRunout.push($.index)
                                if ($.TuanHelpOuts.helpOut.indexOf($.UserName) == -1) $.TuanHelpOuts.helpOut.push($.UserName)
                            }
                        } else {
