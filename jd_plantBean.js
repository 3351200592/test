...
async function requestGet(function_id, body = {}) {
    ...
    if (h5stTool[function_id]) {
        h5st = await getH5st(h5stTool[function_id], h5st_body)
    } else {
        // h5st_body.appid = "wh5"
        h5st_body.appid = "ld"
    }
...
async function taskUrl(function_id, body) {
    let h5st = ''
    let body_in = { ...body }
    if (!body_in.version) body_in.version = "9.2.4.2"
    if (!body_in.monitor_source) body_in.monitor_source = "plant_m_plant_index"
    if (!body_in.monitor_refer) body_in.monitor_refer = ""
    let h5st_body = {
        appid: 'signed_wh5',
        body: JSON.stringify(body_in),
    ...
    let options = {
        url: `https://api.m.jd.com/client.action?functionId=${function_id}&appid=${h5st_body.appid}&body=${encodeURIComponent(JSON.stringify(body_in))}&client=${h5st_body.client}&clientVersion=${h5st_body.clientVersion}${h5st ? "&h5st=" + h5st : ""}`,
        headers: {
            "Cookie": cookie,
            "User-Agent": $.UA,
            "Accept": "*/*",
...
async function getH5st(appid, h5st_body) {
    ...
    try {
        ...
        if (!res) {
            ...
            h5st = res.h5st || ''
            // console.log(decodeURIComponent(h5st))
        }
    } catch (e) {
        console.log(e)
    }
