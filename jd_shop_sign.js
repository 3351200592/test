...
!(async () => {
    ...
    if (token.length == 0) {
        console.log(`店铺token都不给我，跑个锤子哦`)
        return;
    } else console.log(`共有${token.length}个店铺签到活动、开始签到 . . .`)
    ...
    for (var j = 0; j < token.length; j++) {
        ...
        if ($.thismsg) sendmsg += `${$.thismsg}`
    }
    sendmsg = sendmsg.substring(0, sendmsg.lastIndexOf('\n'))
    console.log('\n运行完毕');
    await showMsg()
})()
    .catch((e) => {
...
function getvenderId(token) {
    return new Promise(resolve => {
        ...
        $.get(options, (err, resp, data) => {
            try {
                if (err) {
                    console.log(`❗️获取venderId: API查询请求失败 ‼️‼️`)
                } else {
                    //console.log(data)
                    ...
                    if (data.code == 402) {
                        ...
                        } else console.log(`venderId 获取失败 . . .`)
                    }
                }
            } catch (e) {
                $.logErr(e, resp);
...
function getvenderName(venderId) {
    return new Promise(resolve => {
        ...
        $.get(options, (err, resp, data) => {
            try {
                if (err) {
                    console.log(`❗️getvenderName: API查询请求失败 ‼️‼️`)
                } else {
                    // console.log(data)
                    ...
                    if (data.shopName) {
                        ...
                        message += `${$.shopName}\n`
                    } else console.log(`getvenderName: 获取店铺名失败`)
                }
            } catch (e) {
                $.logErr(e, resp);
            } finally {
...
function getActivityInfo(token, venderId) {
    return new Promise(resolve => {
        ...
        $.get(options, (err, resp, data) => {
            try {
                if (err) {
                    console.log(`❗️获取活动信息: API查询请求失败 ‼️‼️`)
                } else {
                    //console.log(data)
                    data = JSON.parse(/{(.*)}/g.exec(data)[0])
                    if (data && typeof data.data != 'undefined') {
                        ...
                        for (let i = 0; i < data.data.continuePrizeRuleList.length; i++) {
                            let level = data.data.continuePrizeRuleList[i].level
                            let discount = data.data.continuePrizeRuleList[i].prizeList[0].discount
                            console.log(`签到${level}天,可得 ${discount}京豆`)
                            themsg += `\t\t└签到${level}天,可得 ${discount}京豆\n`
                        }
                        if (themsg) message += `${themsg}`
                        console.log("")
                    } else {
                        $.outFlag = true
                        console.log(`活动信息获取失败、可能IP黑了‼️`)
                        message += `\t\t└活动信息获取失败、可能IP黑了\n\n`
                    }
                }
            } catch (e) {
                $.logErr(e, resp);
...
function signCollectGift(token, venderId, activitytemp) {
    return new Promise(resolve => {
        let options = {
            url: `${JD_API_HOST}&t=${Date.now()}&loginType=2&functionId=interact_center_shopSign_signCollectGift&body={%22token%22:%22${token}%22,%22venderId%22:${venderId},%22activityId%22:${activitytemp},%22type%22:56,%22actionType%22:7}&jsonp=jsonp1004`,
            headers: {
                "accept": "accept",
                "accept-encoding": "gzip, deflate",
                "accept-language": "zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7",
        ...
        $.get(options, (err, resp, data) => {
            try {
                if (err) {
                    console.log(`❗️执行店铺签到: API查询请求失败 ‼️‼️`)
                } else {
                    //console.log(data)
                    let errmsg = /msg\":\"(.*?)\",/g.exec(data) || ''
                    let successmsg = /success\":(.*?)}/g.exec(data) || ''
                    data = JSON.parse(/{(.*)}/g.exec(data)[0])
                    if (errmsg) {
                        if (errmsg[1].indexOf('你已经参加过该活动') > -1) {
                            console.log(`今日已签到`)
                        } else if (errmsg[1].indexOf('不存在有效的活动') > -1) {
                            console.log(`活动不存在`)
                            message += `\t\t└活动不存在\n\n`
                            $.outFlag = true
                        } else if (errmsg[1].indexOf('用户达到签到上限') > -1) {
                            console.log(`今日签到已达上限`)
                        } else {
                            console.log(`执行店铺签到1: ${errmsg[1]}`)
                        }
                    } else if (successmsg) {
...
function taskUrl(token, venderId) {
    return new Promise(resolve => {
        ...
        $.get(options, (err, resp, data) => {
            try {
                if (err) {
                    console.log(`❗️获取签到信息: API查询请求失败 ‼️‼️`)
                    message += `\n`
                } else {
                    //console.log(data)
                    data = JSON.parse(/{(.*)}/g.exec(data)[0])
...
function TotalBean() {
    return new Promise(async resolve => {
        ...
        $.get(options, (err, resp, data) => {
            try {
                if (err) {
                    console.log(`❗️TotalBean: API请求失败`)
                } else {
                    if (data) {
                        data = JSON.parse(data);
