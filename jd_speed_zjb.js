...
!(async () => {
    ...
    console.log(`\n\n\n\n======================= 开始助力 =======================`);
    console.log(`默认助力ck1, 可自行填变量: speed_InviterPin 设定邀请码\n`);
    if (process.env.speed_InviterPin) $.InviterPin = process.env.speed_InviterPin;
    if (!$.InviterPin) { console.log(`ck1没获取到邀请码, 请自行填变量再运行脚本`); return }
    if ($.InviterArr[$.InviterPin]) console.log(`下面所有账号将会助力【账号${$.InviterArr[$.InviterPin].index}】${$.InviterArr[$.InviterPin].UserName}\n邀请码为: ${$.InviterPin}\n\n`)
    else console.log(`下面所有账号将会助力: ${$.InviterPin}\n\n`)
        ...
        $.post(options, async (err, resp, data) => {
            try {
                ...
                if (typeof res == 'object') {
                    //console.log(res)
                    if (res.code == 0) {
                        let thisPin = res.data.encryptionInviterPin
                        $.log("邀请码：" + thisPin)
                        if ($.index == 1) $.InviterPin = thisPin
                        $.InviterArr[thisPin] = { "index": $.index, "UserName": $.UserName }
                    } else {
    ...
    return new Promise(async (resolve) => {
        ...
        $.post(options, async (err, resp, data) => {
            try {
                ...
                if (typeof res == 'object') {
                    if (res.code == 0) {
                        console.log(`邀请获得金币: ` + res.data.coinReward * 0.1 + "金币")
                    } else {
                        if (res.message) console.log(res.message)
                        else console.log(data)
                    }
